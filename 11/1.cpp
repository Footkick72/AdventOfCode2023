...
int main() {
    ...
    for (auto g1 = galaxies.begin(); g1 != galaxies.end(); g1++) {
        for (auto g2 = g1+1; g2 != galaxies.end(); g2++) {
            int di = abs(g1->first - g2->first) + (abs(g1->first - g2->first) - abs(bsearch(g1->first, filledis) - bsearch(g2->first, filledis)));
            int dj = abs(g1->second - g2->second) + (abs(g1->second - g2->second) - abs(bsearch(g1->second, filledjs) - bsearch(g2->second, filledjs)));
            ans += di + dj;
        }
    }
    cout << ans << endl;
