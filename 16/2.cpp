...
int main() {
    ...
    for (int i = 0; i < grid.size(); i++) {
        ...
        energize(grid,i,0,0,1,seen,energized);
        best = max(best, (int)energized.size());
        
        seen.clear();
        energized.clear();

        energize(grid,i,grid[0].size()-1,0,-1,seen,energized);
        best = max(best, (int)energized.size());
    }
    for (int j = 0; j < grid[0].size(); j++) {
        ...
        energize(grid,0,j,1,0,seen,energized);
        best = max(best, (int)energized.size());
        
        seen.clear();
        energized.clear();

        energize(grid,grid.size()-1,j,-1,0,seen,energized);
        best = max(best, (int)energized.size());
    }
    std::cout << best << endl;
}